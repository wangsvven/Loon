...
/*
[rewrite_local]
^https?:\/\/.*\/(user\/userInfo\/(get|getExtra|isJxtUser)|help\/common\/getConfig|search\/(getOcrStatus|ocr2|mixPaper)|kaoShi\/.*|paper\/(home|topErrors|list)|user\/coupon\/(getExpireInfo|receiveTips)|mockExam\/config|document\/file|questions\/fetch) url script-response-body https://raw.githubusercontent.com/wangsvven/Loon/refs/heads/main/Scripts/csb.js
...
try {
    ...
    if ((url.includes("user/userInfo/get") || url.includes("user/userInfo/getExtra") || url.includes("user/userInfo/isJxtUser")) && obj.data) {
        ...
        obj.data.free_ad_expiration_time = "2099-09-09 09:09:09";
        obj.data.is_show_ad = "0";
        obj.data.status = "1";
        obj.data.user_type = "2";
        obj.data.phone_code_number = "9999";
        obj.data.traffic_package = "9999999999";
        ...
        if (obj.data.vip_config) {
            ...
            obj.data.vip_config.paid_kaoshi_limit = "9999999";
    ...
    // —— 功能配置 / 广告开关 + 语音VIP ——
    else if (url.includes("help/common/getConfig") && obj.data) {
        if (obj.data.rewarded_video) {
            obj.data.rewarded_video.ios_play_ad_new = "0";
            obj.data.rewarded_video.slide_number = "0";
        }
        if (obj.data.voice_list && Array.isArray(obj.data.voice_list)) {
            obj.data.voice_list.forEach(function (v) {
                v.is_vip = "1";
            });
        }
        if (obj.data.paid_kaoshi) {
            obj.data.paid_kaoshi.is_show = "0";
        }
    }
